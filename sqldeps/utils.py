...
def merge_sql_analyses(analyses: list[SQLAnalysis]) -> SQLAnalysis:
    """Merges multiple SQLAnalysis objects into a single one."""
    merged_dependencies = {}
    merged_outcomes = {}
    ...
    for analysis in analyses:
        ...
        # Merge outcomes
        for table, columns in analysis.outcomes.items():
            if "*" in columns:
                merged_outcomes[table] = {"*"}
            else:
                merged_outcomes.setdefault(table, set()).update(columns)

    return SQLAnalysis(
        dependencies={
            table: list(columns) for table, columns in merged_dependencies.items()
        },
        outcomes={table: list(columns) for table, columns in merged_outcomes.items()},
    )
