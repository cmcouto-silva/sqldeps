...
from sqldeps.utils import merge_schemas, merge_sql_analyses
...
class BaseSQLExtractor(ABC):
    ...
    def extract_from_query(self, sql: str) -> SQLAnalysis:
    ...
    def extract_from_file(self, file_path: str | Path) -> SQLAnalysis:
    ...
    def extract_from_folder(
        ...
        return merge_sql_analyses(dependencies)

    def match_database_schema(
        self,
        dependencies: SQLAnalysis,
    ...
    def _process_response(self, response: str) -> SQLAnalysis:
        """Process the LLM response into a SQLAnalysis object."""
        try:
            # Convert result into a dictionary
            result = json.loads(response)

            if "dependencies" not in result or "outcomes" not in result:
                raise ValueError(
                    "Missing required keys ('dependencies', 'outcomes') "
                    "in the response."
                )

            # Convert dictionary to SQLAnalysis
            return SQLAnalysis(
                dependencies=result["dependencies"], outcomes=result["outcomes"]
            )
